...
function Creature(specie, categorie, predateur,proie,sexe, colorChoosen) {
    ...
    this.degats = calculateRandomValue(10, 80);
    this.position = { x: calculateRandomValue(10,80), y: calculateRandomValue(10,20)}; // 10, 80 normalement.
    ...
    //mieux générer l'id pour éviter les pairs.
...
Creature.prototype.deplacer = function(relation,c1X,c1Y,cProcheX,cProcheY) {
    let id = this.id;
    // encore une organisation pas tres efficace du code surement.
        ...
        if (creature.id !== this.id) { 
            ...
            if(dist === true){
                if(creature.specie === this.predateur){
                    ...
                    this.deplacer(relation,proieX,proieY,predateurX,predateurY); // Ancien 1er parametre = predateur.
                    ...
                    // 1. Calculer la distance entre creature et predateur et faire en sorte qu'elle s'aggrandisse (la distance). 
                    // 2. Prendre les positions du predateur et être dans le + a chacune de ces pos. Si Xpredateur = 10. +x. a quoique.
                    ...
                    this.deplacer(relation,proieX,proieY,predateurX,predateurY) // ancien 1er par = predation.
                    ...
                    if(this.feed > 70 && creature.feed > 70 && this.sexe != creature.sexe && this.mating < 1 && creature.mating < 1){
                        ...
                        if(hit === true){
                            ...
                            if(this.feed < creature.feed){
                                ...
                                // et que les enfants ne puissent pas s'accoupler jusqu'à un certain stade.
...
function newCreature(creature, spe, predateur,proie,sexe, couleur) {
    let creatureS = creature || prompt("Nom de la créature");
    let speS = spe || prompt("Mobile ou immobile");
    let predateurS = predateur || prompt("predateur")
    let proieS = proie || prompt('proie')
    let sexeS = sexe || prompt('sexe ?')
    let couleurS = couleur || prompt("Couleur");
    // a ajouter.
    // Ajouter un bouton + poule, + renard ...
    
    let newCreatureInstance = new Creature(creatureS, speS, predateurS,proieS,sexeS, couleurS);

    creatures.push(newCreatureInstance);
}

function newCreature2(creature, spe, predateur,proie,sexe, couleur) {
    ...
    let newCreatureInstance = new Creature(creature, spe, predateur,proie,sexe, couleur);

    creatures.push(newCreatureInstance);
} // regrouper les 2 plus tard ! ou modifier clairement le nom.

function popC(){
    let newClassicC = new Creature ("classique","mobile", "undefined","undefined","male","pink")
